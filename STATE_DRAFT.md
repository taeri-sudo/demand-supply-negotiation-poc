...
이 문서는 확정본이 아니라, 지금까지 논의를 통해 나온 요소를 한곳에 모은
**검토용 초안**이다. VSCode Claude Code로 넘기기 전에 여기서 먼저 리뷰한다.
설명은 한글, 실제 필드명/태그값(스키마 코드블록 안)은 영어(snake_case).
...
- **procurement_plan·production_plan agent 응답값**: 접근 가능한 실거래
  ...
  양쪽에 재사용
- 위 모든 데이터는 **하나의 인터페이스 함수 뒤에 감춰서** 읽음 — 나중에
