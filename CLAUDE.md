...
엔트리는 항상 행위 주체를 명시해서 쓴다(예: "Claude Code가 X를 발견하지
못했다") — 주어를 생략하거나 "사용자가 ~라고 했는데" 식으로 사용자 행동을
서술하는 문장은 쓰지 않는다. 대화 경위(누가 언제 뭘 물었는지)가 아니라
기록 기준 네 가지(폐기된 시도/가설, 대안 선택 논거와 기각 이유, 재현
절차와 드러난 사실, 예상과 다르게 나온 결과) 자체만 남긴다.
...
- agent 간 상호작용(엣지)을 구현하는 모듈의 파일명·함수명은 `interaction_protocol`의
  ...
  `forecast_supply_round.py`의 `run_forecast_supply_round`). 여러 edge/단계를
